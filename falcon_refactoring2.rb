...
class Show_jl
  ...
  def output
    ...
    #output warnings    
      puts ''  #for readability
      puts 'Warning! No target exists for +' + missing.join(', ') + '.'
      duplicates.each do |duplicate, duplicate_count|
        puts 'Warning! Target not unique: ' + duplicate + ' (' + duplicate_count.to_s + 'x).' 
      end
    end
    #output general warnings
    self.search_arr_object.hints_hash['file_does_not_exist'].each do |file|
